...
<?php if ( have_posts() ) { ?>
	<?php while ( have_posts() ) { 
		...
		if (!empty($tbm_sub_pages)) { ?>
			<?php echo wallow_mobile_seztitle( 'before' ) . __( 'Child pages', 'wallow' ) . wallow_mobile_seztitle( 'after' ); ?>
			<ul class="tbm-group">
				<?php 
		...
		if ( $tbm_the_parent_page ) {?>
			<?php echo wallow_mobile_seztitle( 'before' ) . __( 'Parent page', 'wallow' ) . wallow_mobile_seztitle( 'after' ); ?>
			<ul class="tbm-group">
